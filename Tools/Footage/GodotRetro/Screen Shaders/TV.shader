...
void fragment(){
	...
	float red = texture(TEXTURE, vec2(uv.x + xOffset -0.01*rgbOffsetOpt,_y)).r+staticVal;
	float green = texture(TEXTURE, vec2(uv.x + xOffset,_y)).g+staticVal;
	float blue = texture(TEXTURE, vec2(uv.x + xOffset +0.01*rgbOffsetOpt,_y)).b+staticVal;
